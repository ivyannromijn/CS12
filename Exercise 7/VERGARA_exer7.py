...
def SaveFile(product):
	record= open("petshop_exer7.txt", "w")
	for key in product:
		record.write(key)
		record.write("<#>")
		record.write(product[key]["Brand:"])
		record.write("<#>")
		record.write(product[key]["Type of product:"])
		record.write("<#>")
		record.write(product[key]["Weight of product:"])
		record.write("<#>")
		record.write(product[key]["Number of stock:"])
		record.write("\n")
	record.close()
	print("File saved successfully.") 

def LoadFile(product):
	record = open("petshop_exer7.txt", "r")
	for line in record:
		data = line[:-1].split("<#>")
		product[data[0]] = {"Brand:":data[1],"Type of product:":data[2],"Weight of product:":data[3],"Number of stock:":data[4]}
		record.close
		print("Loaded file")
		return product
...
while True:
	...
	if c==1:
		...
		product= SaveFile(product)
	elif c==7:
		product= LoadFile(product)
	elif c==0:
